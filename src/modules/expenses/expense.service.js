...
class ExpenseService {
  ...
  async createExpense(userId, expenseData) {
    try {
      ...
      if (!account) {
        const err = new Error("Account not found or doesn't belong to user");
        err.statusCode = 404;
        throw err;
      }
      ...
      const wrapped = new Error(`Failed to create expense: ${err.message}`);
  ...
  async getExpenses(userId, options = {}) {
    try {
      ...
      const wrapped = new Error(`Failed to get expenses: ${err.message}`);
      wrapped.statusCode = err.statusCode || 500;
      throw wrapped;
    }
  }
  ...
  async getExpenseById(userId, expenseId) {
    try {
      ...
      const wrapped = new Error(`Failed to get expense: ${err.message}`);
      wrapped.statusCode = err.statusCode || 500;
      throw wrapped;
    }
  }
  ...
  async updateExpense(userId, expenseId, updateData) {
    try {
      ...
      if (!expense) {
        const err = new Error("Expense not found or access denied");
        err.statusCode = 404;
        throw err;
      }
      ...
      const wrapped = new Error(`Failed to update expense: ${err.message}`);
      wrapped.statusCode = err.statusCode || 500;
      throw wrapped;
    }
  }
  ...
  async deleteExpense(userId, expenseId) {
    try {
      ...
      const wrapped = new Error(`Failed to delete expense: ${err.message}`);
      wrapped.statusCode = err.statusCode || 500;
      throw wrapped;
    }
  }
  ...
  _validateAndNormalizeExpenseData(expenseData) {
    const { EXPENSE_CATEGORIES } = require("../../shared/constants/expenses");
